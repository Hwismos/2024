...
class Logger():
    ...
    def render(self):
        ...
        path = 'GREET/' + self.args['dir'] + '/' + time_data[:idx]
...
def main(args):
    ...
    setup_seed(0)
    features, edges, str_encodings, train_mask, val_mask, test_mask, labels, nnodes, nfeats, cur_dataset = load_data(args.dataset)
    results = []

    #! csh
    import sys
    log_args = {'dir': 'out', 'dataset': args.dataset, 'format': 'txt'}
    logger = Logger(log_args)
    print('Writing on file...')

    with open(logger.render(), 'w') as sys.stdout:
        print(f'[{cur_dataset}]')
        print(args)
