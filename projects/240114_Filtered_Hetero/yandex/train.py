...
def get_args():
    ...
    parser.add_argument('--num_steps', type=int, default=1000)  # epoch
...
class MiniTest():
    ...
    def print_dataset_name(self, name):
        print(f"\033[0;30;41m{name}\033[0m")

    def print_dataset_statistics(self, dataset):
        print(f'graph: {dataset.graph}')
        print(f'graph data type: {type(dataset.graph)}')
...
import pickle
def save_obj(obj):
    path = '/home/hwiric/2024/2024_filtered_heterophily_graph_data_test/test.pkl'
...
# csh
def save_feat_and_labels(features, labels):
    file_path = '/home/hwiric/2024/2024_filtered_heterophily_graph_data_test/org_cham_feat_label.txt'
    indicies = (labels == labels).nonzero().flatten().tolist()
    with open(file_path, 'w') as f:
        for idx in indicies:
            str_feats = str(list(map(lambda x: int(x.item()), features[0]))).replace(" ", "")[1:-1]  
            str_line = str(idx) + '\t' + str_feats + '\t' + str(int(labels[idx]))
            f.write(str_line)
...
def main():
    ...
    mt = MiniTest()
    mt.start()
    print(dataset.train_idx_list[0][:20])
    print(dataset.train_idx_list[1][:20])
    mt.end()
