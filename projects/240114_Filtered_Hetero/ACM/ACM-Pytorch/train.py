...
for idx in range(args.num_splits):
    ...
    if args.dataset_name == "deezer-europe":
        ...
        #! csh: fixed_splits 인자와 무관하게 Yandex의 split 데이터
        idx_train = train_idx_yandex[idx].to(device)
        idx_val = val_idx_yandex[idx].to(device)
        idx_test = test_idx_yandex[idx].to(device)
    ...
    #! csh: 각 run별 accuracy를 별도로 출력
    print(f'\033[0;30;43msplit: {idx}, result of each split: {curr_res}\033[0m')
