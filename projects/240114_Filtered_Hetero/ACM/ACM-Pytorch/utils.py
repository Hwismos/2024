...
# csh
MYPATH = '/home/hwiric/2024/2024_filtered_heterophily_graph_data_test/ACM'
...
class MiniTest():
    ...
    def start(self):
        print(f'\033[0;30;46mTEST STARTS\033[0m')

    def end(self):
        print(f'\033[0;30;46mTEST ENDS\033[0m')
        exit()
...
def data_split(idx, dataset_name):
    splits_file_path = "ACM/ACM-Pytorch/splits/" + dataset_name + "_split_0.6_0.2_" + str(idx) + ".npz"
...
def load_full_data(dataset_name):
    if dataset_name in {"cora", "citeseer", "pubmed"}:
        ...
        graph_adjacency_list_file_path = os.path.join(
            MYPATH, "new_data", dataset_name, "out1_graph_edges.txt"
        )
        graph_node_features_and_labels_file_path = os.path.join(
            MYPATH, "new_data", dataset_name, "out1_node_feature_label.txt"
        )
        ...
        cur_dataset = dataset_names[4]
        print(f'\033[0;30;46m{cur_dataset}\033[0m')
        dataloader_yandex = YandexDataLoader(cur_dataset)
        G_yandex = dataloader_yandex.save_dgl_to_nx()

        train_idx = dataloader_yandex.train_idx_list
        val_idx = dataloader_yandex.val_idx_list
        test_idx = dataloader_yandex.test_idx_list

        adj = nx.adjacency_matrix(G_yandex, sorted(G_yandex.nodes()))
        # ===========================================================
        ...
        features = np.array(
            [
                features
                for _, features in sorted(G_yandex.nodes(data="features"), key=lambda x: x[0])
            ]
        )
        labels = np.array(
            [label for _, label in sorted(G_yandex.nodes(data="label"), key=lambda x: x[0])]
        )        
...
def train_prep(logger, args):
    ...
    #! csh: split 인덱스도 같이 반환
    adj_low_unnormalized, features, labels, train_idx, val_idx, test_idx = load_full_data(args.dataset_name)
    ...
    return (device, model_info, run_info, adj_high, adj_low,
            adj_low_unnormalized, features, labels, split_idx_lst,
