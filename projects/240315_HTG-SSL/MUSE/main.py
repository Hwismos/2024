...
class Model(nn.Module):
    def __init__(self, input, hidden, output, tau, drop, dropout_mlp, gate):
...
def train(model, optimizer, x1, x2, adj1, adj2, drop_edge_index, args, deg):
    ...
    loss_gnn.backward()  # loss_{contrast}
    optimizer.step()
    ...
    mlp_loss = hetero_loss + args.alpha4 * L2_loss + args.alpha3 * reg
...
logger.time_str()
logger.path_str()
file_name = logger.file_name() + '_' + args.dataset + '.txt'
msg = 'File writing...'
...
with open(file_name, 'w') as sys.stdout:
    ...
    for run in range(args.runs):
        ...
        model = Model(input, hidden, output, tau, drop, dropout_mlp, gate)
        model = model.to(device)
        # print(model)

        optimizer = torch.optim.Adam([
                                {'params':model.params, 'lr':args.lr, 'weight_decay':args.weight_decay},
                                {'params':model.params_mlp, 'lr':args.lr_mlp, 'weight_decay':args.wd_mlp},
                                ])
        ...
        for epoch in range(args.epochs):
            ...
            logger.train_print(run, epoch, gnnloss, mlploss)
        ...
        if args.task == 'node_classification':
            ...
            logger.test_print(run, best_micro)
    ...
    if args.task == 'node_classification':
        ...
        logger.final_print(best_test_micro_f1.mean(), best_test_micro_f1.std(),
                           best_test_macro_f1.mean(), best_test_macro_f1.std(), args)
