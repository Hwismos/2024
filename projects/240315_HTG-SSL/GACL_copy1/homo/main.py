...
if __name__ == '__main__':
    ...
    logger.time_str()
    logger.path_str()
    file_path = logger.path_without_dataset + args.dataname + '.txt'
    f = open(file_path, 'w+')
    f.write('[ARGUMENTS]: {}\n'.format(args))
    ...
    for run in range(0, args.run_times):
        ...
        for epoch in range(1000):
            ...
            with th.no_grad():
                ...
                f.write('[EPOCH]: {}, [TRAIN ACC]: {:.4f}, [VAL ACC]: {:.4f}, [TEST ACC]: {:.4f}\n'.format(epoch, train_acc, val_acc, test_acc))
        # print(f'Validation Accuracy: {best_val_acc}, Test Accuracy: {eval_acc}')
        f.write('[VALIDATION ACC]: {:.4f}, [TEST ACC]: {:.4f}\n'.format(best_val_acc, eval_acc))
